...
function drawGanttChart(ganttData) {
    ...
    Highcharts.ganttChart('gantt_chart', {
        series: [{
            name: 'Tareas',
            data: ganttData.map((task, index) => {
                return {
                    start: task[3].getTime(),
                    end: task[4].getTime(),
                    name: task[1],
                    y: index  // Asignando un índice único a cada tarea
                };
            })
        }],
        title: {
        ...
        yAxis: {
            type: 'category',
            categories: ganttData.map(task => task[1]), // Nombres de las tareas para el eje Y
            title: {
                text: ''
            }
        }
